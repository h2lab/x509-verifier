...
int x509_cert_extract_tbs_and_sig(const u8 *buf, u16 len,
				  const u8 **tbs_start, u16 *tbs_len,
				  const u8 **sig_alg_start, u16 *sig_alg_len,
				  const u8 **sig_start, u16 *sig_len)
{
	const _alg_id *sig_alg;
	u16 seq_data_len = 0;
	u16 eaten = 0;
	u16 remain;
	int ret;
	...
	*tbs_len = eaten;
	...
	ret = parse_x509_signatureAlgorithm(buf, remain, sig_alg, &eaten);
	if (ret) {
		ERROR_TRACE_APPEND(__LINE__);
		goto out;
	}
	...
	*sig_alg_len = eaten;
	...
	/* Parse third element of the sequence: signatureValue */
	ret = parse_x509_signatureValue(buf, remain, sig_alg, &eaten);
	if (ret) {
		ERROR_TRACE_APPEND(__LINE__);
		goto out;
	}
